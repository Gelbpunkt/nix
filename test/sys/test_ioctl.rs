...
mod linux {
    ...
    fn test_op_write_64() {
        if cfg!(any(target_arch = "mips64", target_arch = "powerpc64")) {
            assert_eq!(
                request_code_write!(b'z', 10, (1 as u64) << 32) as u32,
                0x8000_7A0A
            );
        } else {
            assert_eq!(
    ...
    fn test_op_read_64() {
        if cfg!(any(target_arch = "mips64", target_arch = "powerpc64")) {
            assert_eq!(
                request_code_read!(b'z', 10, (1 as u64) << 32) as u32,
                0x4000_7A0A
            );
        } else {
            assert_eq!(
    ...
    fn test_op_read_write_64() {
        assert_eq!(
            request_code_readwrite!(b'z', 10, (1 as u64) << 32) as u32,
            0xC000_7A0A
        );
    }
}
...
mod bsd {
    ...
    fn test_op_read_write_64() {
        assert_eq!(
            request_code_readwrite!(b'z', 10, (1 as u64) << 32),
            0xC000_7A0A
        );
    }
}
...
mod linux_ioctls {
    ...
    use nix::errno::Errno::{ENOSYS, ENOTTY};
    use nix::Error::Sys;
