...
use nix::errno::*;
use nix::sys::aio::*;
use nix::sys::signal::{sigaction, SaFlags, SigAction, SigHandler, SigSet, SigevNotify, Signal};
...
fn poll_aio(aiocb: &mut AioCb) -> Result<()> {
    loop {
        let err = aiocb.error();
        if err != Err(Error::from(Errno::EINPROGRESS)) {
            return err;
        };
...
fn test_accessors() {
    ...
    let aiocb = AioCb::from_mut_slice(
        1001,
        2, //offset
        &mut rbuf,
        42, //priority
...
fn test_cancel() {
    ...
    let mut aiocb = AioCb::from_slice(
        f.as_raw_fd(),
        0, //offset
        wbuf,
        0, //priority
...
fn test_aio_suspend() {
    ...
    let mut rcb = AioCb::from_mut_slice(
        f.as_raw_fd(),
        8, //offset
        &mut rbuf,
        0, //priority
...
fn test_read() {
    ...
    f.write_all(INITIAL).unwrap();
    {
        let mut aiocb = AioCb::from_mut_slice(
            f.as_raw_fd(),
            2, //offset
            &mut rbuf,
            0, //priority
...
fn test_write_bytes() {
    ...
    let mut f = tempfile().unwrap();
    f.write_all(INITIAL).unwrap();
    let mut aiocb = AioCb::from_boxed_slice(
        f.as_raw_fd(),
        2, //offset
        wbuf,
        0, //priority
...
fn test_read_bytes_mut_small() {
    ...
    let mut aiocb = AioCb::from_boxed_mut_slice(
        f.as_raw_fd(),
        2, //offset
        rbuf,
        0, //priority
...
#[cfg(not(any(target_os = "ios", target_os = "macos")))]
#[cfg_attr(
    any(target_arch = "mips", target_arch = "mips64", target_env = "musl"),
    ignore
)]
...
fn test_liocb_listio_read_immutable() {
    ...
    let mut liocb = LioCb::from(vec![AioCb::from_slice(
        f.as_raw_fd(),
        2, //offset
        rbuf,
        0, //priority
